...
const [searchQuery, setSearchQuery] = useState('');
 ...
 const filteredPatients = donePatients?.filter((patient) => {
  ...
  const handleDelete = async (id) => {
    const isConfirmed = window.confirm("Are you sure you want to delete this patient detail?");
    if (isConfirmed) {
      try {
      ...
      <div className="main">
          ...
          <div className="row flex-nowrap">
            ...
            <div className="col-xxl-11 col-xl-11 col-lg-11 col-md-11 col-sm-11" style={{ marginTop: "5rem" }}>
              <IoArrowBackSharp className="fs-1 text-black d-print-none mx-2" onClick={goBack} style={{ cursor: "pointer" }} />
...
<tbody>
    ...
    <>
      ...
      <tr key={patient.testid}>
        ...
        <td>{patient.test}</td>
                              <td className="text-success fw-bold">{patient.test_status}</td>
                              <td>
                                <div className="">
                                  <Link to={`/final-oral-testing/${patient.testid}`}>
                ...
                <nav>
                  <ul className="pagination">
                    ...
                    <li
                      className={`page-item ${currentPage === Math.ceil(filteredPatients.length / itemsPerPage) ? "disabled" : ""}`}
                    >
                      <button
                        className="page-link"
                        onClick={() => paginate(currentPage + 1)}
